...
class DownloadClickViewSetTests(APITestCase):
    def setUp(self):
        self.test_user = User.objects.create_superuser('test_user',
                                                       'test@user.com',
                                                       'password')

        self.data = {'userid': self.test_user.id,
        ...
        self.data['tokenid'] = self.download_token.id

        self.client.login(username='test_user', password='password')

    def test_create_downloadClick(self):
        response = self.client.post(reverse('mediaviewer:api:downloadclick-list'), self.data)
