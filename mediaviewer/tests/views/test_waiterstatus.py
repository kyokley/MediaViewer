...
class TestAjaxWaiterStatus(TestCase):
    def setUp(self):
        ...
        settings_patcher = mock.patch("mediaviewer.views.waiterstatus.settings")
    ...
    def test_no_status(self):
        ...
        self.mock_requests.get.assert_called_once_with(
            "test_url", timeout="test_request_timeout"
        )
        self.mock_dumps.assert_called_once_with(
            {"status": False, "failureReason": "Bad Symlink"}
        )
        self.mock_new.assert_called_once_with(False, "Bad Symlink")
    ...
    def test_good_status(self):
        ...
        self.mock_dumps.assert_called_once_with({"status": True, "failureReason": ""})
    ...
    def test_request_timed_out(self):
        ...
        self.mock_dumps.assert_called_once_with(
            {"status": False, "failureReason": "Timedout"}
        )
        self.mock_new.assert_called_once_with(False, "Timedout")
        self.mock_HttpResponse.assert_called_once_with(
            self.mock_dumps.return_value, content_type="application/javascript"
        )
