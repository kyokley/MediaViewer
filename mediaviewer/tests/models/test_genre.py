...
class TestGetGenres:
    ...
    def setUp(self, create_path, create_file, create_poster_file):
        ...
        # File posters
        create_poster_file(file=self.tv_file1,
                           genres=[self.thriller,
                                   self.mystery])
        create_poster_file(file=self.tv_file2,
                           genres=[self.thriller,
                                   self.action])
        create_poster_file(file=self.tv_file3,
                           genres=[self.comedy,
                                   self.action])
        create_poster_file(file=self.tv_file4,
                           genres=[self.drama,
                                   self.thriller])

        # Path posters
        create_poster_file(path=self.tv_path1,
                           genres=[self.action,
                                   self.mystery])
        create_poster_file(path=self.tv_path2,
                           genres=[self.thriller,
                                   self.action])
        create_poster_file(path=self.tv_path3,
                           genres=[self.comedy,
                                   self.action])
        create_poster_file(path=self.tv_path4,
                           genres=[self.drama,
                                   self.thriller])

        # Movie posters
        create_poster_file(file=self.movie_file1,
                           genres=[self.thriller,
                                   self.mystery])
        create_poster_file(file=self.movie_file2,
                           genres=[self.thriller,
                                   self.action])
        create_poster_file(file=self.movie_file3,
                           genres=[self.thriller,
                                   self.action])
        create_poster_file(file=self.movie_file4,
                           genres=[self.mystery,
                                   self.drama])
