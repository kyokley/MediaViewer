...
class PathViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        newPath = (
            Path.objects.filter(localpathstr=data["localpath"])
        ...
        if not newPath:
            ...
            return RESTResponse(
                serializer.data, status=RESTstatus.HTTP_200_OK, headers=headers
            )
...
class TvPathViewSet(PathViewSet):
    ...
    def get_queryset(self):
        ...
        try:
            finished = query_param_to_bool(self.request.query_params.get("finished"))
...
class DistinctTvPathViewSet(viewsets.ViewSet):
    def list(self, request):
        ...
        serializer = TvPathSerializer((paths[key] for key in paths), many=True)
        return RESTResponse(serializer.data)
