...
def _decode_auth_header(headers):
    basic_auth = base64.b64decode(headers["Authorization"].split()[-1]).decode("utf-8")
...
def legacy_verify(request):
    ...
    if user is None:
        ...
        return JsonResponse(
            {"user_id": user.pk, "nickname": user.username, "email": user.email}
        )
...
def verify_email(request):
    if request.method == "PUT":
        ...
        data = _user_info_from_body(request.body)
        UserSettings.objects.filter(user__email__iexact=data["email"]).update(
            verified=True
        )
        return JsonResponse({})
