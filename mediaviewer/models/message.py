...
class Message(models.Model):
    ...
    message_type = models.CharField(
        max_length=15, choices=MESSAGE_TYPES, default=REGULAR, null=False, blank=True
    )
    ...
    levelDescriptions = ["Debug", "Info", "Success", "Warning", "Error"]
    ...
    def createLastWatchedMessage(cls, user, file, level=messages.WARNING):
        ...
        cls.createNewMessage(user, body, level=level, message_type=LAST_WATCHED)

    @classmethod
    def clearLastWatchedMessage(cls, user):
        for message in cls.getMessagesForUser(user, message_type=LAST_WATCHED):
            message.delete()
