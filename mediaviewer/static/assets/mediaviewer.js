const passkey_client = new Passwordless.Client({
  apiKey: "mediaviewer:public:8cd7916568ca470cb0738f8ce8d20f18",
});
...
function prepareDataTable($, sortOrder, table_data_page, filter_id) {
  if (filter_id) {
    var ajax_path =
      "/mediaviewer/ajax/" + table_data_page + "/" + String(filter_id) + "/";
  } else {
...
function dataTableConfig($, sortOrder, table_data_page, ajax_path) {
  ...
  if (table_data_page == "ajaxtvshows") {
    ...
    responsive_priorities = [{ responsivePriority: 500, target: 1 }];
  } else if (table_data_page == "ajaxtvshowssummary") {
    responsive_priorities = [{ responsivePriority: 1, target: 0 }];
...
async function bypass_passkey() {
  bypass_path = document.location.pathname.replace(
    "user/reset",
    "bypass-passkey",
  );
